...
import { CacheStore, cacheSnapShot, collectionSnapShot, EntityCache, EntityType } from "./entity-cash";
 ...
 */
export function addEntity<T extends EntityType>(
  entity: T,
 ...
 * has all and only the properties required, and its FKs (if any) are valid.
 */
export function updateEntityGuard<T extends { id: number }>(
  entity: T,
  collectionName: keyof EntityCache,
  cache$: Observable<EntityCache>
): T {
  ...
  const bad = null == collectionSnapShot(collectionName, cache$).find((e: { id: number }) => entity.id === id);
  if (bad) {
    throw new Error(
